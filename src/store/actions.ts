 ...
 * @param context
 */
export const initComponentList = ({ commit }, node) => {
  commit('initComponentList', node);
};
export const SettingChange = ({ commit }, node) => {
  commit('SettingChange', node);
};

export const setConfig = ({ commit }, cfg) => {
...
export const setCurrentPage = ({ commit, state }, page) => {
...
export const setPageInfo = ({ commit, state }, pageInfo) => {
  let info = Object.assign({}, state.pageInfo, pageInfo);
  commit('SET_PAGE_INFO', info);
};

export const dataHubSet = ({ commit, state }, { path, val }) => {
