...
function getBaseNode(node) {
  ...
  const idCache = Object.keys(window.$_nodecomponents || {}).concat([
    node.name,
  ]);
  console.log(info, 'kkk2', JSON.stringify(info));
...
function modifyNodeId(
  ...
  function random(node) {
    ...
    if (
      !node[key] ||
      (isInclude(idCache, node[key]) && !isInclude(exclude, node[key])) ||
      isInclude(localIdCache, node[key])
    ) {
  ...
  function genKey(base) {
    ...
    while (!(key && !isInclude(localIdCache, key))) {
      key = `${base || ''}${((Math.random() * 1296) | 0)
        .toString(36)
        .padStart(2, '0')}`; // 两位36进制数
    }
    return key;
  }
  return random(tree);
...
function getNoRotateBoundingClientRect(el) {
  ...
  var pos = {
    bottom:
      innerHeight - el.offsetHeight - topAndLeft.top - margins.marginBottom,
...
function dimensionAnyTopx(el, parent) {
  ...
  if (isRoot || elPosition === 'fixed')
    parent = document.getElementById('stage');
  var [parentRect, rect] = [parent, el].map((el) =>
    getNoRotateBoundingClientRect(el),
  );
...
function confirmWithGoodbye(msg, goodbyeKey) {
  ...
  if (window.localStorage.getItem(goodbyeKey) == 1)
...
function trbl(str = '', key = '') {
  ...
  const [top, right, bottom, left] = arr.concat(
    { 1: [arr[0], arr[0], arr[0]], 2: [arr[0], arr[1]], 3: [arr[1]], 4: [] }[
      arr.length
    ] || [],
  );
  return {
